...
from kiku.db.store import get_tinder_queue, save_tinder_decision
...
def tinder_queue(
    ...
    if track_ids:
        try:
            parsed_ids = [int(x.strip()) for x in track_ids.split(",") if x.strip()]
        except ValueError:
            pass
...
def tinder_retrain(db: Session = Depends(get_db)):
    ...
    from kiku.analysis.autotag import save_model, train_energy_model
