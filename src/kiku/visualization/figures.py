...
from kiku.analysis.waveform import envelope_to_time_axis
...
def _load_waveform(
    ...
    if use_detail:
        ...
        duration = (
            len(np.frombuffer(audio_features.waveform_detail, dtype=np.float32)) * hop / sr
            if audio_features.waveform_detail
            else len(envelope)
        )
...
def build_track_figure(
    ...
    if waveform_data is None:
        fig.add_annotation(
            text="No waveform data. Run: kiku analyze --waveform-only",
            xref="paper",
            yref="paper",
            x=0.5,
            y=0.5,
            ...
            fig.add_trace(
                go.Scattergl(
                    ...
                    marker=dict(
                        ...
                        size=8,
                        color="#533483",
                        line=dict(width=1, color="#533483"),
                    ),
                    name="Beats",
    ...
    if show_energy and audio_features.energy_intro is not None:
        ...
        energy_y = (
            [audio_features.energy_intro] * 2
            + [audio_features.energy_body] * 2
            + [audio_features.energy_outro] * 2
        )
        fig.add_trace(
            go.Scatter(
                x=energy_x,
    ...
    for st in sorted(set_tracks, key=lambda s: s.position):
        ...
        if cumulative_time > 0:
            fig.add_vline(
                ...
                annotation_text=f"{st.transition_score:.2f}"
                if st.transition_score and show_transitions
                else None,
...
def build_staircase_figure(
    ...
    for i, st in enumerate(sorted_tracks):
        ...
        if af and af.waveform_overview:
            ...
            max_env = float(np.max(envelope)) if len(envelope) > 0 else 1.0
            scaled = (
                (envelope / max_env * lane_height * 0.7) + y_offset
                if max_env > 0
                else np.full_like(envelope, y_offset)
            )
            ...
            fig.add_trace(
                go.Scattergl(
                    ...
                    if "rgb" in color
                    else f"rgba({int(color[1:3], 16)}, {int(color[3:5], 16)}, {int(color[5:7], 16)}, 0.25)",
                    line=dict(width=0),
                    showlegend=False,
                    hoverinfo="skip",
                )
    ...
    if energy_profile is not None:
        try:
            ...
            if isinstance(energy_profile, EnergyProfile):
                ...
                y_pts = [energy_profile.target_energy_at(t / 60) * abs(y_min) for t in x_pts]
                fig.add_trace(
                    go.Scatter(
                        x=x_pts.tolist(),
                        y=y_pts,
    ...
    if waveform_a is not None and mapped_a is not None:
        ...
        if cues_a:
            ...
            shifted_cues = [
                ...
                for c in cues_a
                if c["start_sec"] >= show_from_raw_a
            ]
    ...
    if waveform_b is not None and mapped_b is not None:
        ...
        if cues_b:
            ...
            shifted_cues = [
                ...
                for c in cues_b
                if c["start_sec"] <= show_until_raw_b
            ]
...
def _add_transition_markers(
    ...
    fig.add_vline(
        x=a_end_time,
        line=dict(color="#e94560", width=1, dash="dash"),
        annotation_text="A ends",
        annotation_font=dict(size=9, color="#e94560"),
    )
    fig.add_vline(
        x=b_start_on_timeline,
        line=dict(color="#00d2ff", width=1, dash="dash"),
        annotation_text="B starts",
        annotation_font=dict(size=9, color="#00d2ff"),
    )
