    ...
    for i in range(1, 3):
        s.add(
            Track(
                ...
                artist="Astral",
                album="Solar EP",
                file_path=f"/nope/Astral/Solar EP/0{i}.flac",
                track_number=i,
            )
        )
    s.commit()
...
def test_falls_through_itunes_to_deezer(session):
    deezer = [
        {"title": "Solar EP", "artist": {"name": "Astral"}, "cover_xl": "https://e-cdn/xl.jpg"}
    ]
...
def test_caa_used_when_mb_release_known_before_external(session):
    session.add(
        AlbumMetadata(album_key=KEY, album="Solar EP", album_artist="Astral", mb_release_id="rel-1")
    )
    session.commit()
